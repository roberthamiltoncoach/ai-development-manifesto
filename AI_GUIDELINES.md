...
- **Structured Logging & Traceability:** All logs must be structured as JSON. Every network request, background cron, or database mutation must generate and pass a unique correlation_id through the entire stack to allow the AI to immediately trace front-end UI errors back to the exact failing backend function.
...
- **What to ask the AI:** When dealing with dependency errors, use this prompt constraint:
  > _"Analyze this error log. What is the blast radius of fixing this? Do not attempt to update dependencies until you have summarized the potential breaking changes and I have approved them."_
...
- **Schema-Driven API Contracts:** Backend endpoints and frontend clients must not rely on duplicated or loosely defined types. They must share a single source of truth (e.g., OpenAPI schemas or shared tRPC/GraphQL types). Breaking changes to API schemas must fail the build before deployment.
...
- **Shift-Left Security & Secret Prevention:** Pre-commit hooks must include rigorous automated secret scanning (e.g., gitleaks) to ensure API keys, SSH keys, or access tokens are never committed.
- **Data Sanitization & Privacy:** Personally Identifiable Information (PII) must never be written to application logs. Implement automated redaction utilities for logging payloads. Any local emulator database seeds must use strictly anonymized or synthetically generated data.

## 6. The "Zero to One" Initialization Rule

- **Foundational Integrity First:** Before writing _any_ feature code for a new repository, the AI must ensure that the foundational production values are active. This means setting up a basic CI/CD pipeline (e.g., GitHub Actions), a Dependabot configuration (`.github/dependabot.yml`) to monitor package managers and GitHub Actions, enforcing a code formatter (Prettier), a linter (ESLint), and pre-commit hooks (Husky).
...
- **Health-Check Driven Rollbacks:** Deployment pipelines must include post-deployment health checks. If critical API endpoints fail to return a 200 OK, or if error rates spike beyond a predefined threshold within 5 minutes of a deployment, the CI/CD pipeline must automatically trigger a zero-touch rollback to the previous stable release.
...
- **Zero-Trust Execution:** AI agents must operate under the Principle of Least Privilege. Destructive commands (e.g., rm -rf, database drops, or production ssh access) must require explicit, out-of-band developer approval or be executed in isolated, ephemeral Docker containers. Production deployments must be routed through CI/CD pipelines governed by strict IAM roles, rather than executed locally via the CLI by the AI.
...
- **Simultaneous Edit Collision Prevention:** To prevent overwriting the developer's active work, the AI must verify the target file's current state (via timestamp or hash) before executing file writes. If the file has been modified manually by the developer during the AI's generation cycle, the AI must abort the write and prompt the developer to resolve the conflict.
